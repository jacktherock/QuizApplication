...
def user_signup(request):
    if not request.user.is_authenticated:
        ...
        messages.error(
            request, "You Already Logged In ! Please Logout to Create New Account !"
        )
...
def user_login(request):
    if not request.user.is_authenticated:
        ...
        messages.error(
            request,
            "You Already Logged In ! Please Logout to Login from another Account !",
        )
        return HttpResponseRedirect("/")
...
def delete_user(request, id):
    if request.user.is_authenticated:
        if request.user.is_superuser:
            ...
            return HttpResponseRedirect("/auth/authadmin/allusers/")
        else:
            messages.error(request, "Unauthorized User Can't Access This Page !")
            return redirect("/")
    else:
